...
import com.languify.communication.conversation.dto.ConversationRequestDTO;
import com.languify.communication.conversation.dto.ConversationResponseDTO;
...
import com.languify.communication.conversation.service.ConversationTranscriptionService;
...
import com.languify.infra.realtime.RealtimeEventHandler;
import com.languify.infra.websocket.Handler;
...
public class ConversationHandler extends Handler {
  ...
  public void handleSegment(String segment, JsonNode data, WebSocketSession session) {
    ...
    switch (segment) {
      case "start":
        try {
          ConversationRequestDTO.Start dto = this.mapper.treeToValue(data, ConversationRequestDTO.Start.class);
      ...
      case "data":
        try {
          ConversationRequestDTO.Data dto =
              this.mapper.treeToValue(data, ConversationRequestDTO.Data.class);
  ...
  private void startConversation(ConversationRequestDTO.Start data, WebSocketSession session) {
    Session s = this.extractSessionFromWebSocketSession(session);
        ...
        new Realtime(
            ...
            new RealtimeEventHandler() {
              ...
              public void onAudioDelta(String audioDelta) {
                if (firstDelta) {
                  emit(
                      ...
                      new ConversationResponseDTO.TranslateState("reproducing"),
                      userId,
                      session);
                ...
                emit(
                    ...
                    new ConversationResponseDTO.AudioDelta(audioDelta),
                    userId,
                    session);
              }
              ...
              public void onTranslationDone() {
                emit(
                    "conversation:translate:state",
                    new ConversationResponseDTO.TranslateState("ready"),
                    userId,
                    session);
                ...
                state.ifPresent(s -> s.getRealtime().clearBuffer());
              }
            });
  ...
  private void processConversationData(ConversationRequestDTO.Data data, WebSocketSession session) {
    Session s = this.extractSessionFromWebSocketSession(session);
    UUID userId = s.getUser().getId();
  ...
  this.emit("conversation:translate:state", new ConversationResponseDTO.TranslateState("loading"), userId, session);
  }
  ...
  public void cleanupConnectionState(WebSocketSession session) {
    Session s = this.extractSessionFromWebSocketSession(session);
