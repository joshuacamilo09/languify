...
public interface ChatRepo extends JpaRepository<Chat, Long> {
  ...
  @Query(
      "SELECT c FROM Chat c WHERE (c.user1.userId = :userId OR c.user2.userId = :userId) AND c.active = true")
  List<Chat> findActiveChatsByUserId(@Param("userId") Long userId);

  // Buscar mensagem específica em um chat
  @Query(
      "SELECT m.Originaltext FROM ChatRecord m WHERE m.chat.chat_id = :chatId AND m.id = :messageId")
